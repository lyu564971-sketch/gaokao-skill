...
## ADR-008 | LLM API Key 使用约束 + 模型限制（免费白名单）
- **日期**：2026-06-20
- **状态**：已采纳
- **背景**：用户提供智谱 GLM API Key 用于线上部署，明确要求只使用免费模型。
- **决策**：
  1. **API Key 信息**（敏感，仅记录于治理文件，禁止提交到公开仓库）：
     - Provider：`glm`
     - Base URL：`https://open.bigmodel.cn/api/paas/v4`
     - API Key：`b877746c417c496c8f44e574cd20eb01.oU9wpqAwq1iWRTeZ`
  2. **模型白名单**（只允许使用以下免费模型，禁止任何付费模型）：
     - ✅ `glm-4.5-air`（主力模型，当前默认）
     - ✅ `glm-4-flash`（如需更快速响应时可选）
     - ❌ 禁止：`glm-4.6`、`glm-4-plus`、`glm-4-long`、`glm-4-alltools` 等所有付费模型
  3. **API Key 保管规则**：
     - 仅存储在 Render 环境变量和本治理文件中
     - 禁止硬编码到源码、禁止提交到 Git（`.gitignore` 已包含 `.env`）
     - `.env.example` 中的 API Key 占位值为 `your-api-key-here`，不含真实值
- **理由**：
  1. 用户明确要求免费模型，使用付费模型将产生费用，违反用户约束；
  2. API Key 是用户财产，需妥善保管，泄露可能导致滥用扣费；
  3. 将约束写入治理文件，确保任何协作者（含 AI agent）都能看到并遵守。
- **后果**：
  1. `render.yaml` / Render 环境变量中 `LLM_MODEL` 必须为 `glm-4.5-air` 或其他免费模型；
  2. 任何代码变更、模型切换前，必须检查目标模型是否在白名单内；
  3. 如白名单需更新（智谱发布新免费模型），在本 ADR 补充，禁止绕过。
